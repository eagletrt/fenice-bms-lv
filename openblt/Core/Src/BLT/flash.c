...
#define FLASH_END_ADDRESS \
    (flashLayout[FLASH_TOTAL_SECTORS - 1].sector_start + flashLayout[FLASH_TOTAL_SECTORS - 1].sector_size - 1)
...
blt_bool FlashWrite(blt_addr addr, blt_int32u len, blt_int8u *data) {
    ...
    if ((FlashGetSector(addr) == FLASH_INVALID_SECTOR) || (FlashGetSector(addr + len - 1) == FLASH_INVALID_SECTOR)) {
        return BLT_FALSE;
    }
...
blt_bool FlashWriteChecksum(void) {
   ...
   */
    ...
    /* write the checksum */
    return FlashWrite(
        flashLayout[0].sector_start + BOOT_FLASH_VECTOR_TABLE_CS_OFFSET,
        sizeof(blt_addr),
...
static blt_bool FlashEraseSectors(blt_int8u first_sector, blt_int8u last_sector) {
    ...
    if ((first_sector < flashLayout[0].sector_num) || (last_sector > flashLayout[FLASH_TOTAL_SECTORS - 1].sector_num)) {
        result = BLT_FALSE;
    }
